Slugcoin Core version *0.15.1* is now available from:

  <https://download.slugcoin.org/slugcoin-0.15.1/>
...
A number of changes to the way Slugcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 slugs per kB (0.00001 SLG). This is to prepare for dropping the minimum transaction fee to 0.1 slugs per kB (0.0001 SLG) in the next release.
