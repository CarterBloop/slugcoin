...
The Slugcoin Core daemon can be configured to act as a trusted "border
router", implementing the slugcoin wire protocol and relay, making
...
The ZeroMQ feature in Slugcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building slugcoind:
...
For instance:
    ...
    $ slugcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/slugcoind.tx.raw \
...
These options can also be provided in slugcoin.conf.
...
From the perspective of slugcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into slugcoind directly. Furthermore, no information is
...
using. Slugcoind appends an up-counting sequence number to each
