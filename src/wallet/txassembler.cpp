    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional SLG outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes SLG and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::SLG_TO_SLG) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a SLG-to-SLG transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::SLG_ONLY;
        ...
        // If SLG-to-SLG fails, create a peg-out transaction
